...
a1 = np.deg2rad(30)
a2 = np.deg2rad(30)
circle1 = np.row_stack((  a1*np.ones((1,100))  ,np.linspace(0,2*np.pi,100)))
circle1_cart = ori_ang2cart(circle1)
rot1 = rodrigue(np.array([1,1,0]),np.deg2rad(30))
# print(circle_cart)
# print(circle)
...
circle2 = np.row_stack((  a2*np.ones((1,100))  ,np.linspace(0,2*np.pi,100)))
circle2_cart = ori_ang2cart(circle2)
rot2 = rodrigue(np.array([2,1,0]),np.deg2rad(-20))
# print(circle_cart)
# print(circle)
...
rr = np.sqrt(np.divide( np.square(np.cos(a1))+np.square(np.cos(a2))-2*np.multiply(np.multiply(np.cos(a1),np.cos(a2)),np.cos(ang_diff[0,0])) ,np.square(np.sin(ang_diff[0,0]))))
ang_side = np.arccos(rr)
ang_1to2 = np.arccos(np.cos(a1)/rr)
axis_1to2 =np.array( [ \
                      (np.sin(o1_ori[0,:])*np.cos(o1_ori[1,:])*np.cos(o2_ori[0,:])- np.cos(o1_ori[0,:])*np.sin(o2_ori[0,:])*np.sin(o2_ori[1,:])) ,\
                      (np.cos(o1_ori[0,:])*np.sin(o2_ori[0,:])*np.cos(o2_ori[1,:])- np.cos(o2_ori[0,:])*np.sin(o1_ori[0,:])*np.cos(o1_ori[1,:])) , \
                      (np.sin(o1_ori[0,:])*np.sin(o2_ori[0,:])*np.sin(o2_ori[1,:]-o1_ori[1,:])) ]  )
axis_1to2 = np.cross(o1[:,0],o2[:,0])
mid = np.matmul(rodrigue(axis_1to2,ang_1to2),o1)
axis_side = np.cross(mid[:,0],axis_1to2)


p1 = np.matmul(rodrigue(axis_side,ang_side),mid)
p2 = np.matmul(rodrigue(axis_side,-ang_side),mid)
'''--------------------------------------------------------------------------------------------------'''
...
# =============================================================================
# a,b,c = ori_ang2cart(pd_ref_ori)
# ax.scatter(a,b,c,marker='x',c='r')
# a,b,c = ori_ang2cart(pd_other_ori)
# ax.scatter(a,b,c,marker='x',c='b')
# a,b,c = ori_ang2cart(ori_tar)
...
ax.scatter(circle1_flat[0,:],circle1_flat[1,:])
ax.scatter(o1_f[0,:],o1_f[1,:])
a = stereo_3dto2d(mid)
ax.scatter(a[0,:],a[1,:],marker='x')
ax.scatter(circle2_flat[0,:],circle2_flat[1,:])
ax.scatter(o2_f[0,:],o2_f[1,:])

a,b = stereo_3dto2d(p1)
ax.scatter(a,b,marker='x')
a,b = stereo_3dto2d(p2)
ax.scatter(a,b,marker='x')
