...
def solve_mulmul(X):
    ...
    led_m = X[0]
    pd_m = X[1]
    pd_ori_ang = np.stack((X[2:2+led_num],X[2+led_num:2+2*led_num]))
    led_ori_ang = np.stack((X[2+2*led_num:2+2*led_num+pd_num],X[2+2*led_num+pd_num:2+2*led_num+2*pd_num]))
